...
**关键节点 slot 约定**：
- `FUNC_DECL`: S1=名称, S2=返回类型, S3=类型参数, List=参数, I1=函数体, I3=1 if private method (D068), I4=annotations
- `CLASS_DECL`: S1=类名, S2=父类名, List=字段, I2=方法块
- `VAR_DECL`: S1=变量名, S2=CONST/LET, S3=类型标注, I1=初始值
- `PARAM`: S1=参数名, S2=类型, S3="const"（字段级 const 标记）, I1=默认值, I2=isOptional, I3=1 if private field (D068)
...
- **Private members**: `private` keyword for class fields/methods — compile-time access control (D068)
