...
**决策记录**：每个确认的设计决策立即写入 `docs/3-decisions/D0NN-*.md`，一个决策一个文件，不等到实现完成再补。多阶段计划的 Phase 进度只写在 D 文档里。下轮提示词输出到对话或写入 `.claude/next_prompt.md`（由 `tools/send_next.ss` 通过 terman bracketed paste 注入下一轮，见 `docs/terman-auto-next.md`），两种方式都可，但 handoff 文件仅作为 send_next.ss 的 payload，不做跨轮状态累积。
