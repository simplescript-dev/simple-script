...
**状态:** Done at `lib/json.ss:286 jnRoot` + `lib/json.ss:294-309 jnGetInt/Double/String/Bool` + `lib/spring/boot/application.ss:42 RouteMeta.httpMethod` + `lib/spring/boot/application.ss:60 PARAM_KIND_REQUEST_BODY` + `lib/spring/boot/application.ss:67-71 comptime 5 method` + `lib/spring/boot/application.ss:90-96 RequestBody push spec` + `lib/spring/boot/application.ss:177-178 dispatcher httpMethod 比对` + `bootstrap/eval/method_call.ss:159-185 invoke sentinel RequestBody 分支` + `bootstrap/gen/gen_type_ops.ss:206 deserializerTargets Map` + `bootstrap/gen/gen_type_ops.ss:215-228 emitPendingDeserializers` + `bootstrap/gen/gen_type_ops.ss:401-451 emitClassDeserializeFn` + `bootstrap/gen/codegen.ss:269-273 reset` + `bootstrap/gen/codegen.ss:331-333 emit hook` + `examples/spring-parity/hello/ss/HelloController.ss + java HelloController.java POST /users` + `tests/phase5/i021_requestbody.ss 6 case` (2026-04-25 next_prompt option a + sub-option b 单轮端到端 Execute,bootstrap 三阶段固定点 + 6 i021_requestbody.ss case + 5 i021_pathvariable.ss + 4 i021_multi_param.ss backward compat 全 GREEN + parity 端到端 curl POST 3 次连续 RC retain 验通无 use-after-free + reflection_health_linter D123#扩容申报-I021-requestbody bump-group 5 metric record sync GATE PASS)
