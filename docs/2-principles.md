...
3. **下一步提示词**：最后一条回复**直接输出**下一步简短提示词（1-3 句、单段、命令式、模仿用户原始风格），**同时**覆盖写入 `.claude/next_prompt.md`(`tools/send_next.ss` 的单次 payload,见 `docs/terman-auto-next.md`)。**对话 + 文件两路缺一不可**——对话让用户审阅措辞,文件让 terman 一键触发下轮,省去手动复制粘贴的 UX 开销。两处内容严格一致;Phase 进度仍只落在 D 文档,`.claude/next_prompt.md` 不承载跨轮状态累积 / 进度摘要
