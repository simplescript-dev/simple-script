...
| 9 (b) | Backfill | **待实施(下轮 / 拆轮)** — audit-driven backfill 副产物(linter 跑后自动列 candidates: C5 220 / C6 6 / C8 43 / C9 3 = 272 candidates → 人工 audit 标 SUNSET / PERMANENT / false positive → 逐项 backfill;**这是协议扩的自然副产物,不是人工记忆 18 处**,反 workaround 反模式 — sibling D097 拆 commit 模板:decision spec → linter 实现 → bump CLI / backfill 三 commit) | 大改 | bootstrap/ + tools/ + docs/ 多子族(D 文档反向锚加 + 工具/代码侧 SUNSET marker 加 + 部分清/PERMANENT 替换) |
