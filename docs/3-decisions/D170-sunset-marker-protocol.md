...
**Status:** **Phase 0 Done at f3a93bd** — D170 §决策落地;**Phase 1 全闭环 Done at f3a93bd→3ff649a→72f47c3→d7b5d9c** (Phase 0 立项 + sunset_linter MVP + MNK gate 接入 + CLAUDE.md 强制段,三层 SSoT 链);**Phase 2 起首 Done at 9ca0e0e** — 9 处显式过渡件 SUNSET marker backfill (eval_expr.ss UNARY/BINARY/COMPTIME_EXPR + ct_driver + interp_op + stmts_loop_classic for/while + pow_binary/null_coalesce);**Phase 2 续 In Progress at step 5b** (23 处类 A 批量 backfill — gen_decls (5) + gen_assigns (2) + gen_methods (1) + gen_exprs (3) + stmts_branch/exc/enum/loop_forin/core/simple/stmts (8) + class_comptime + class (2) + stmts_enum (1) + 1 method,本轮 — 32 markers 全库 GATE OK);**Phase 2 完待 step 6** (sunset_linter --phase X + Phase Exit Gate)。
...
| 5b | Backfill | **本 commit** — 23 处类 A 批量 backfill (D093 §0.4 Phase 2-5 candidates,13 文件) + 32 markers 全库 GATE OK + bootstrap 三阶段固定点 verified | 大改 | bootstrap/ 单子族 |
...
- **[~] In Progress Phase 2 续 (step 5b)** — 23 处类 A 批量 backfill (D093 §0.4 Phase 2-5 candidates,跨 13 文件):**Phase 2** stmt 族 8 处 (`stmts/stmts.ss` EXPR_STMT + `stmts_core.ss` break/continue + `stmts_loop_forin.ss` ct-fold post-check + `stmts_branch.ss` if/switch + `stmts_exc.ss` try/throw + `stmts_simple.ss` postfix) + **Phase 3** decl/assign 族 7 处 (`gen_decls.ss` funcDecl/destructure[Array/Object]/varDecl/return + `gen_assigns.ss` member/general assign) + **Phase 4** call/expr 族 4 处 (`gen_methods.ss` resolveSuperParent + `gen_exprs/exprs.ss` THIS/SUPER + ARROW_FUNC + 主 dispatch 入口) + **Phase 5** class/enum 族 3 处 (`stmts_enum.ss` registerEnum + `class_comptime.ss` ctLookupCurrent + `class.ss` genClassDecl);**32 markers 全库 GATE OK** + bootstrap 三阶段固定点 verified Stage 2 = Stage 3 + bin/ss 字节同 + 全测 baseline 334/3 持平;全库自然语言「留 X+」/「Phase Y 留」类过渡件**全部清零**,D170 §第一性需求"过渡债漂移防"实质覆盖度 100%
