...
| 7 | Implementation/实战 | Done at **b94dc8b** (C1 feat impl) + **609b465** (C2 docs exit verify) — 回流 `feat/d092-sema-q1` 主线 + D093 §Phase 1.5d sibling 子轮 (for/while 迁) 首例 SUNSET marker 实战 + Phase Exit Gate manual verify 首次实操; **sibling 第二例应用 Done at b0f8d12 (C1) + f9360f9 (C2)** — D093 §Phase 1.5e+ ext (pow_binary + null_coalesce eager 上移桥消除 + genNullCoalesce 接口扩 lPreReg/rPreReg) + 2 SUNSET marker 物理清 + §出口清单 段 sibling 第二例落地 (默认模式 verify) | 大改 | bootstrap/ + docs/ + tests/ 拆 2 commit (合 D170 §决策 C 独立 commit 原则);sibling 第二例同 2 commit 拆 |
