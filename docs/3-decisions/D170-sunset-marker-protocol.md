...
**Status:** **Phase 0 Done at f3a93bd** — D170 §决策落地;**Phase 1 起首 Done at 3ff649a** — tools/sunset_linter.ss MVP C1-C4 + 4 doc-anchor 测试 + simplify 3 项落实;**Phase 1 续 Done at 72f47c3** — docs/3-MNK.md §特定领域 §SUNSET marker 治理 gate + §After Done §1.5;**Phase 1 完 Done at d7b5d9c** — CLAUDE.md §项目技术规则「SUNSET marker (强制)」段 + 双链 D170 §决策 + MNK §SUNSET marker 治理 gate (用户级 SSoT 入口,三层 SSoT 链 Phase 1 全闭环);**Phase 2 起首 In Progress at step 5a** (8/9 处显式过渡件 SUNSET marker backfill — bootstrap/eval/eval_expr.ss UNARY/BINARY/COMPTIME_EXPR + ct_driver enter/exit + interp_op interpValEquals NaN/精度 + stmts_loop_classic for/while + pow_binary/null_coalesce eager 上移,本轮);**Phase 2 续待 step 5b** (~40 处类 A 批量);**Phase Exit Gate 落地待 step 6**。
...
| 5a | Backfill | **本 commit** — 9 处显式过渡件 SUNSET marker 落地 (实证表 #1-7 + #6 sibling 拆 2 + #7 拆 2 = 9 markers, 6 文件 bootstrap/eval+gen/stmts) + sunset_linter GATE OK + bootstrap 三阶段固定点 verified + 全测 baseline 334/3 持平 | 大改 (多文件 bootstrap/) | bootstrap/ 单子族 |
...
- **[~] In Progress Phase 2 起首 (step 5a)** — 9 处显式过渡件 backfill SUNSET marker (本 D 文档 §历史语境 表 #1-7 + #6 sibling 拆 for+while 2 markers + #7 拆 pow_binary+null_coalesce 2 markers,合计 9):`eval_expr.ss` UNARY 反向编码 (D093 §Phase 1.5e+) + `eval_expr.ss` BINARY 混合 dispatch (D093 §Phase 1.5e+) + `eval_expr.ss` COMPTIME_EXPR 类 C (D093 §Phase 8) + `ct_driver.ss` enter/exit dual-write (D093 §Phase 8) + `interp_op.ss` interpValEquals NaN/精度 (D098 §Phase C) + `stmts_loop_classic.ss` for/while (D093 §1.5d 主轮收口子步 sibling 子轮) + `pow_binary.ss`/`null_coalesce.ss` eager 上移 (D093 §Phase 1.5e+);sunset_linter GATE OK 9 markers all valid + bootstrap 三阶段固定点 verified Stage 2 = Stage 3 (SS 注释 lex 丢弃 token stream 同) + 全测 baseline 334/3 持平
