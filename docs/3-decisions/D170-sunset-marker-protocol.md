...
- **[~] In Progress step 7 实战 起首 (本 commit C1 feat impl)** — Phase Exit Gate 协议**实战首例**(D170 协议设计→工具→流程→实战四阶段闭环最后一步):`bootstrap/gen/stmts/stmts_loop_classic.ss:12+72` for/while 入口同形 sibling 迁(`comptimeDepth > 0 → comptimeMustBeKnown == 1` 字面 rename + silent `||` 短路拆 loud `comptimeError("for|while condition not compile-time known", id)`,与 do-while ddd327c 完全一致)+ **2 SUNSET marker 物理清**(line 12 for + line 72 while,sibling 子轮 phase 完结后 markers 不再需要)+ 2 新 spike doc-anchor `tests/phase5/comptime_{for,while}_unknown_error.ss.txt`(sibling do-while spike 完全一致 — bare IDENT cond 触 loud error)+ bootstrap 三阶段固定点 verified + 全测 baseline 334/3 持平 + sunset_linter 默认 30 markers GATE OK(-2 from 32)+ `--phase` 模式 verdict PASS "no cleanup needed";**扩容申报-step-7-实战-bump-fix**:本轮顺手补 95e0bab/d8a010b 漏 bump 历史遗留 — `F1:bootstrap/gen/gen_decls.ss` 745→750(95e0bab step 5b Phase 3 SUNSET markers ×5 加在 gen_decls.ss line 35/360/455/530/687 时漏申报,本轮 step 7 一并补 bump)
- **[ ] Planned step 7 配套 C2 (docs exit verify)** — D093 §下一步 sibling 子轮 [ ] Planned → [x] Done at C1 hash + D093 add §Phase 1.5d sibling 子轮 §出口清单 段(2 行 [clean] markers)+ D170 §下一步 [x] step 7 + commit footer "Phase 1.5d 主轮收口子步 sibling 子轮 exit verify" + 标准「去掉少什么:」

**回流到 `feat/d092-sema-q1`**:6 步完结后,本分支 rebase / merge 回主线;之后 1.5d sibling 子轮 (for/while 迁) 直接用 SUNSET marker 取代自然语言「留 X+」首批落地。step 7 实战已落地,本轮 C1 feat impl 同分支完成,C2 docs exit verify + git merge fast-forward 回 feat/d092-sema-q1。
