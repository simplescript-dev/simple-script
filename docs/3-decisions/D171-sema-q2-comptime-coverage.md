...
- **[x] Done — I036 全局非字面量 double 标量物化未支持致 llc 硬失败修复(2026-05-29 follow-up,`global_double_scalar_materialize.options.md` GATE 6/6,候选 B 接口层 trap,`.bugfix` 6 gate PASS)**。RED 全局 `let g=1.5+2.5` → llc `store ptr 0x4010000000000000, ptr @gAdd` "floating point constant invalid for type" 硬失败(`grep -c 'error: llc'`=2,0x4010..=4.0 的 IEEE754 位;**行 171 末 carve-out「无标注全局 double 物化 … → 立项 I036」兑现** — materialization **物化侧** gap,与 I035 propagation 侧正交,= global_scalar_materialize int/bool 物化家族的 double 扩展)。根因(单文件 2 站点,gen_decls.ss 全局标量物化接口边界,与 int/bool 完全同构仅类型差):**站点 A** `genGlobalVar` else 分支 realType=int/bool 已分派 `global i32 0`、**realType=double 无分支**落最终 else `@x=global ptr null`(ptr slot,值是 double);**站点 B** `emitGlobalInits` gVarType=int/bool 已分派 `store i32`、**double 无分支**落 else `store ptr <doubleval>`,而 genExpr 对 double 算术返 `fadd double` 寄存器 / 折叠常量返 `0x<16hex>` 精确位(interp_value.ss:39)/ array-get 返 `bitcast i64→double` 寄存器 / double-call 返 `call double` 寄存器 → `store ptr <doubleval>` → llc 类型错。根因修复=站点 A `realType=="double"` → `@x=global double 0.0, align 8`(初值 0.0 因 LLVM 拒整数 `global double 0`;与同函数 COMPTIME_EXPR double 分支 `gen_decls.ss:260` 物化模板对齐)+ gType=realType;站点 B 对称 `store double`(读取侧 `genIdent` 经 `getVarType→ssTypeToLLVM` 自动 `load double`,COMPTIME_EXPR double 全局 + 局部 alloca double 已验此路径),**无需额外 `.0` guard**(折叠值 `0x<hex>` 前缀在 constVal 源头承担,store double 操作数恒合法),零新 runtime IR/函数。GREEN:全局 double 算术(add/sub/mul/div)+ 引用其他全局 + 无标注 double 数组 array-get + double-fn-call 全编译通过 + 显示 parity(整数值 double "4"/"6"、非整 "6.5"/"-3.5"、"2.5"/"3.14")+ assertApprox 值断言 + comptime==runtime parity(`comptime{return 1.5+2.5}` / array-get oracle);回归 `tests/phase5/global_double_scalar_materialize.ss`(18 assert)。VCM §3:当前 bin/ss(未含 fix)run test → llc `store ptr 0x4010000000000000, ptr @gAdd` ↔ /tmp/newcc(fixed)+ bootstrap 后 bin/ss all pass。bootstrap 三阶段固定点 + 全测 349→350(+1,3 pre-existing 不变)+ reflection_health GATE PASS(F1 gen_decls 763→775 走 **§扩容申报-I036** bump 765→780,合法特性增长;N2/N3/N4 soft drift 不 bump)+ sunset GATE OK 无新 marker + d_doc/derived_issue GATE OK + `.bugfix` 6 gate PASS。net_new_ifs=2 / net_new_fns=0 / same_pattern_count=0(全局非字面量 double 物化唯一 2 站点,均已对称补;COMPTIME_EXPR double 分支独立既存,无第三残留)。**范围外(独立根因,不混入):comptime double 物化 = 既有 I011(comptime 侧,本 I036 是 runtime 全局侧);Map.getInt I003b deferred 维持**。**D171 §收口验收 runtime-parity 族「行为级穷举 parity 零硬失败」全清(I033 字面量 / comptime-return / I034 comparison-logical / global_scalar_materialize int/bool 标量物化 / I035 元素类型传播 / 本 I036 double 标量物化 — 全局 int/bool/double 物化 3 类型对称),合并就绪**。

- **[x] Done — 合并就绪巡检 + I034 编号冲突修复（2026-05-30）**:bootstrap 三阶段定点 ✅ + 全测 350/3 ✅(3 pre-existing 实锤 spring_web_params/harness_task/d096_p4_l2_reactive)+ reflection/sunset/d_doc/derived/bugfix 全 gate clean ✅ + Phase 1-5 / runtime-parity 族全 Done + backlog（I011 comptime double / I003b Map.getInt）正确 scope-out。**巡检发现 1 处 doc 治理 gap**:I034 编号被「comparison/logical→bool（已 Done,行 169）」与「array-elem-infer backlog」双重占用 → 立项 `I037` + 兑现其 §推荐:array-elem-infer 迁 `I038`、comparison 工作补 canonical `I034-comparison-logical-bool.md`、`I037` 闭环,源码↔issue 语义对齐(零改 `bootstrap/`)。bugfix gate G5 一度因 `bin/ss run` 持有 bin/ss 致 cp ETXTBSY 误报,改文档规定 `build -o /tmp/X && /tmp/X` 后 6/6 PASS(定点本身始终验证通过)。
...
## 扩容申报-I036-global-double-scalar-materialize（reflection_health_linter）

global 非字面量 **double** 标量物化修复（gen_decls.ss 2 站点对称物化 double slot）落地后 `tools/reflection_health_linter.ss` 1 项**硬 REGRESSION**（F1 `gen_decls.ss`），为**合法特性增长**：全局 double 算术 / 无标注 double 数组 array-get / double-fn-call 物化是 D171 §收口验收 runtime-parity 族「行为级穷举 parity 零硬失败」收口**最后一块**（int/bool 标量物化已由 §扩容申报-global-scalar-materialize 覆盖、元素类型传播已由 I035 覆盖，唯 double 标量物化未扩），补此能力须在 `genGlobalVar` else 分支 + `emitGlobalInits` 各加 double 分派（net +12 行，与 int/bool slice 同构）。

**§第一性需求关联**：D171 §收口验收「行为级穷举 parity，零静默误编译 / 零硬失败」+ §第一性需求「comptime = 完整语言」的 runtime 对侧；不补则顶层 `let g = 1.5 + 2.5` 编译失败（语言基本能力残缺，与 int/bool/double 局部全 GREEN 不对称）。该增长是能力内在成本，非八股（反身性：去掉则整类全局非字面量 double 不可编译，回退 `store ptr <double>` → llc 崩）。

**delta 表**（cur = 本轮后实测）：

| 指标 | 定义 | HEAD(I035 后) | cur | 我的 slice | 旧 bm(来源) | 新 bm | 性质 |
|---|---|---|---|---|---|---|---|
| **F1 gen_decls.ss** | 文件行数 | 763 | 775 | +12 | 765(global-scalar-materialize) | **780** | 2 站点 double 物化分派（genGlobalVar else + emitGlobalInits）合法增长 |
| N2 | 节点字节估算 | — | ~433300 | ~0 | 432000(D168) | 不 bump | AUTO-DRIFT soft（1% 内，主 pre-existing 累积，非本轮 +12 行所致） |
| N3 | AST 深度总和 | — | ~593500 | ~0 | 592000(D171-Ph4) | 不 bump | AUTO-DRIFT soft（0.25% 内，主 pre-existing 累积） |
| N4 | 最大出度 | 360 | 360 | +0 | 350 | 不 bump | SCOPE-DRIFT soft（diff 未触反射白名单；baseline pre-existing > bm，本轮 gen_decls 嵌套分支不增 max 出度） |

**本地抵消路径（A 路径不可达）**：2 站点 double 分派是物化 double slot 的最小代码集（声明 + store 各 1 分支），削则阉割特性（回退 `global ptr null` + `store ptr <double>` → llc "floating point constant invalid for type" 崩）；拆 `gen_decls.ss` 为 12 行不值当（文件职能内聚：函数 decl + 全局 var + var decl + return + destructure）。**MNK §B.2 形态 8 禁止压缩注释挤 F1 阈值**（近阈值正解 = 拆文件或 bump，不改字符密度）→ 保留根因注释（同 §扩容申报-global-scalar-materialize int/bool slice 注释密度），走 **B 路径扩容申报**只 bump F1 gen_decls 1 项硬 REGRESSION；soft AUTO/SCOPE-DRIFT（N2/N3/N4，主 pre-existing 累积非本轮）留设计豁免不 bump。

**预估 vs 实测**：bump 后 cur ≤ 新 bm（F1 gen_decls 775≤780），re-run gate 预期 PASS（实测见 VCM §1）。
