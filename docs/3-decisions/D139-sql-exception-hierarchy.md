...
9. **SS typed try/catch/throw class extends 完整支持 — 不修编译器**:`bootstrap/lexer/lexer.ss:454-456` TRY/CATCH/THROW keywords + `bootstrap/parse/parse_stmts.ss:59 parseTryCatch / 96 parseThrow` 完整 parser + `bootstrap/checker/check_stmts.ss:425 TRY / 455 THROW` typed catch 类型检查 + `bootstrap/gen/stmts/stmts_exc.ss:23 genTryCatch / 172 genThrow` codegen + `tests/phase5/typed_catch.ss` 6 case + `throw_error.ss` 4 case 实证 GREEN — 本 D 范围**应用层 stdlib 纯 SS,不动编译器**(对照 D138 §核心原则 7 编译器扩允许是 H4 假设破裂回路;本 D 已 spike 实证 SS 全支持,不需要)
...
- `lib/com/mysql/query.ss` 删:`const RESULT_SET_HEADER_ERR = -1`(line 23)+ `function readUpdateResult(fd: int): int` thin wrapper(line 135-137)+ `function parseOkPacketAffectedRows(payload, payloadLen): int` thin wrapper(line 108-110)— D138 §核心原则 5 退役
- `lib/com/mysql/query.ss` 改:`parseResultSetHeader / readResultSetHeader / parseOkPacket / readUpdateResultPacket` ERR / short read / unrecognised header → throw SQLException 子类(SQLState class 反查 → 具体子类)
- `lib/com/mysql/handshake.ss` 改:`mysqlConnect` 删 errMsg + println 路径 → throw SQLNonTransientConnectionException(SQLState `08xxx` connection / `28xxx` invalid auth)
- `lib/com/mysql/jdbc.ss` 改:`MysqlConnection.commit / rollback / setAutoCommit` 升级 throw + `MysqlStatement.executeUpdate / executeQuery / getGeneratedKeys` 升级 throw + 删 ERR-path sentinel `affectedRows == -1` 守卫(D138 §A.2 H7)
- `lib/com/mysql/prepared.ss` 改:`MysqlPreparedStatement.executeUpdate / executeQuery / getGeneratedKeys` 同位升级 throw
- SQLState class 反查 → SQLException 子类 dispatch:
  - `08xxx` → SQLNonTransientConnectionException
  - `23xxx` → SQLIntegrityConstraintViolationException
  - `40xxx` → SQLTransactionRollbackException
  - `42xxx` → SQLSyntaxErrorException
  - `0Axxx` → SQLFeatureNotSupportedException
  - `22xxx` → SQLDataException
  - `28xxx` → SQLNonTransientConnectionException(invalid auth)
  - `HY000` → SQLException 默认
  - 其他 → SQLException 默认
- `tests/d134_mysql/`(commit/rollback/setAutoCommit/query_test 调用方)+ `tests/d135_caching_sha2/` + `tests/d136_prepared_statement/` + `tests/d138_generated_keys/`(Case 6 ERR packet 防御从 `affectedRows == -1` 改 catch SQLException)调用方更新 try/catch
- spike Phase 2:`INSERT INTO ... dup_key VALUES (1,'A')` 撞 unique 索引 catch SQLIntegrityConstraintViolationException + sqlState 23000 + errorCode 1062 GREEN
- VCM 验证:bootstrap 三阶段固定点 + tests/d134-d138 baseline 升级后不降(全测 baseline 不降)+ reflection_health PASS + d_doc_index PASS

### Phase 3: Spring DAE 完整树 + SQLExceptionTranslator [ ] Pending at commit `<Phase 3 commit>`

- `lib/spring/jdbc.ss` 加 Spring DAE 完整树 15+ 子类:
  - 顶层:`class DataAccessException extends Error { cause: SQLException }`(Spring `org.springframework.dao.DataAccessException`)
  - 中间:`TransientDataAccessException extends DataAccessException` / `NonTransientDataAccessException extends DataAccessException`
  - NonTransient 子树:`DataIntegrityViolationException` / `BadSqlGrammarException` / `CannotGetJdbcConnectionException` / `DataAccessResourceFailureException` / `DataRetrievalFailureException`
  - DataIntegrity 子树:`DuplicateKeyException extends DataIntegrityViolationException`
  - DataRetrieval 子树:`EmptyResultDataAccessException` / `IncorrectResultSizeDataAccessException`
  - Transient 子树:`TransientDataAccessResourceException` / `ConcurrencyFailureException`
  - Concurrency 子树:`DeadlockLoserDataAccessException extends ConcurrencyFailureException`
- `lib/spring/jdbc.ss` 加 `class SQLExceptionTranslator { translate(sqlException: SQLException): DataAccessException }`(简化 Spring `org.springframework.jdbc.support.SQLErrorCodeSQLExceptionTranslator`)反查表:
  - **MySQL errorCode 主映射**(优先于 SQLState):
    - 1062 → DuplicateKeyException
    - 1048 → DataIntegrityViolationException(NOT NULL)
    - 1452 → DataIntegrityViolationException(FK)
    - 1146 → BadSqlGrammarException(table not exist)
    - 1054 → BadSqlGrammarException(column unknown)
    - 1213 → DeadlockLoserDataAccessException
    - 1205 → ConcurrencyFailureException(lock wait timeout)
    - 1045 → CannotGetJdbcConnectionException(invalid auth)
  - **SQLState 前 2 char fallback**:
    - `08` → CannotGetJdbcConnectionException(connection failure)
    - `23` → DataIntegrityViolationException(integrity)
    - `40` → DeadlockLoserDataAccessException(rollback)
    - `42` → BadSqlGrammarException(syntax)
    - `28` → CannotGetJdbcConnectionException(invalid auth)
  - **默认 fallback**:DataAccessResourceFailureException
- spike Phase 3:`translator.translate(new SQLIntegrityConstraintViolationException("Duplicate entry", "23000", 1062))` 返 DuplicateKeyException + 5 层继承 catch hierarchy 实证(`catch (e: DuplicateKeyException) → catch (e: DataIntegrityViolationException) → catch (e: NonTransientDataAccessException) → catch (e: DataAccessException)`)
- VCM 验证:bootstrap 三阶段固定点 + tests/d134-d138 baseline 不破 + reflection_health PASS + d_doc_index PASS

### Phase 4: JdbcTemplate 上层 catch / translate [ ] Pending at commit `<Phase 4 commit>`

- `lib/spring/jdbc.ss` JdbcTemplate.update / query / queryForXxx / queryForObject(D137 5 method + D138 第 6 method)全 method 上层 try / catch SQLException → SQLExceptionTranslator.translate → throw DAE — Spring 标准
- 内部 per-call Connection 路径(D137/D138):prepareStatement → setter → executeUpdate / getGeneratedKeys → 全在 try block 内;catch (e: SQLException) → translator.translate(e) → throw DAE;close 在 finally
- spike Phase 4:`try { jdbc.update("INSERT INTO ... dup_key VALUES (1)") } catch (e: DuplicateKeyException) {...}` Spring 标准 GREEN(业务侧 catch DAE 子类不见底层 SQLException)
- VCM 验证:bootstrap 三阶段固定点 + tests/d134-d138 baseline 不破 + reflection_health PASS + d_doc_index PASS
