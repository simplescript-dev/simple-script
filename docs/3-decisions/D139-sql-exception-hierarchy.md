...
### Phase 4: JdbcTemplate 上层 catch / translate [✓] Done at commit `a9155a3`
...
### Phase 5: 测试覆盖 7 形态 + 全 Phase 收关 [✓] Done at commit `81de9bd` — D139 主线 close

- 重写 `tests/d139_sql_exception/integration_test.ss`(D139 §核心目标 7 判据全形态 e2e 实证,7 个 `test()`)+ docker MySQL 探活 fallback skip(沿 d134/d136/d138/Phase 2+4 spike 模式 — `bin/ss test tests/` 离线零阻塞);URL `jdbc:mysql://root:test@127.0.0.1:3307/testdb`(沿 D134 docker fixture)+ 表名 `d139_sql_exception_test`(隔离 D134 users / D136 users_d136 / D138 d138_generated_keys / Phase 2 d139_dup_test / Phase 4 d139_phase4_dup 并行批量)
- **Case 1**(connection 失败 wrong password,`integration_test.ss:70-87`):URL `jdbc:mysql://root:wrongpass@127.0.0.1:3307/testdb` → handshake ERR_PACKET → SQLNonTransientConnectionException(协议层)→ JdbcTemplate translate(errorCode 1045 优先映射)→ catch CannotGetJdbcConnectionException + cause.sqlState 28xxx + cause.errorCode 1045 — D139 §核心目标 第 1 判据 + §A.2 H1/H7
- **Case 2**(INSERT duplicate key,`integration_test.ss:90-112`):`INSERT IGNORE` seed + 同 id 再 INSERT → catch DuplicateKeyException + cause.sqlState 23000 + cause.errorCode 1062 + ex.message 含 "Duplicate entry" — D139 §核心目标 第 2 判据 + §A.2 H3 实证(吸收 Phase 2 spike + Phase 4 spike Test 1 同等)
- **Case 3**(table 不存在,`integration_test.ss:114-130`):`INSERT INTO d139_does_not_exist (id) VALUES (1)` → catch BadSqlGrammarException + cause.sqlState 42S02 + cause.errorCode 1146 — D139 §核心目标 第 3 判据
- **Case 4**(NOT NULL violation,`integration_test.ss:132-150`):`INSERT INTO d139_sql_exception_test (id, name) VALUES (2, NULL)` 触发 `name VARCHAR(100) NOT NULL` → catch DataIntegrityViolationException + cause.sqlState 23000 + cause.errorCode 1048 — D139 §核心目标 第 4 判据
- **Case 5**(SQLException 三字段 raw java.sql layer,`integration_test.ss:152-182`):`conn.prepareStatement.executeUpdate` bypass JdbcTemplate translate → catch (ex: SQLException) 取 ex.message + ex.sqlState + ex.errorCode 三字段全可达(协议层 `dispatchSQLException` 23xxx → SQLIntegrityConstraintViolationException 透传 ErrorPacket 三字段)— D139 §核心目标 第 5 判据 + §A.2 H5 实证
- **Case 6**(5 层 catch hierarchy,`integration_test.ss:184-234`):同 dup_key 5 次独立 throw,分别被 (6a) DuplicateKeyException leaf / (6b) DataIntegrityViolationException 中间 / (6c) NonTransientDataAccessException 祖父 / (6d) DataAccessException 根(`ex.cause.errorCode == 1062` 仍可达)/ (6e) SS prelude `Error` 顶层(DAE extends Error)catch — D139 §核心目标 第 6 判据 + §A.2 H1 + H6 跨 module class hierarchy multi-level fallthrough 实证(吸收 Phase 4 spike Test 2 4 层 + 扩 NonTransientDAE 中间 + Error 顶层 5 层完整)
- **Case 7**(handshake 失败 wrong user,`integration_test.ss:236-251`):URL `jdbc:mysql://wronguser:wrongpass@127.0.0.1:3307/testdb` → catch CannotGetJdbcConnectionException + cause.sqlState 28000 + cause.errorCode 1045(MySQL "Access denied for user" 标准 — handshake → ERR_PACKET → JdbcTemplate translate 路径同 Case 1)— D139 §核心目标 第 7 判据
- **absorb 决策**:删 `tests/d139_sql_exception/phase4_spike_test.ss`(Phase 4 spike Test 1 jdbc.update dup_key e2e 已被 Case 2 吸收 + Test 2 4 层 catch hierarchy 已被 Case 6 5 层吸收 + Test 3 finally-close 担保 socket 不饥饿是 Phase 4 主轴行为已落 `lib/spring/jdbc.ss:166-394` 11 method);原 Phase 2 spike `integration_test.ss` Case 2 单 case 已被本 Phase 重写覆盖 — 7 case 主轴 vs spike 重叠 case 维护负担消除,违反"Spring 主线一文件标准"风险消除;**保留** `tests/d139_sql_exception/translator_spike_test.ss` 12 case 纯本地无 docker(Phase 3 SQLExceptionTranslator 反查表 8 路径 + 4 层 hierarchy 实证),与 e2e 7 case 协议层 vs 业务层互补**无重叠**
- 编译器零改动(本 Phase 纯 docs/ + tests/);SS arrow-function closure capture rebinds outer-scope catch `e`(§Followup F10)workaround 沿用:7 case + 6 sub-block 内层 catch 全用 `ex` 命名,外层 probe 才用 `e`
- **§Phase 收关锚 §Phase 5 mark Done — 全 Phase 0-5 commit hash 6 个总览**:
  - Phase 0 `68a8184` — D 文档落档(D138 §Followup F2 + §A.3 废案 §C3 SQLException 部分 起首)
  - Phase 1 `04821ce` — 协议层 ErrorPacket(`lib/com/mysql/query.ss:131-161`)+ JDBC SQLException 12 类 hierarchy(`lib/java/sql.ss:114-159`)+ spike 跨 module 3 层 GREEN
  - Phase 2 `dd0ef4a` — 协议层全 throw + 删 sentinel(`RESULT_SET_HEADER_ERR` + `readUpdateResult` + `errMsg+println` 全删)+ 调用方 commit/rollback/setAutoCommit/executeXxx 全升级 + INSERT dup_key spike + 编译器 1 处扩(`bootstrap/gen/stmts/stmts_exc.ss:172-205` byte-offset GEP fix)
  - Phase 3 `be394f5` — Spring DAE 14 子类(`lib/spring/jdbc.ss:60-108`)+ SQLExceptionTranslator 反查表(`:117-145` MySQL errorCode 8 + SQLState 5 char + 默认 fallback)+ translator_spike 12/12 GREEN 纯本地
  - Phase 4 `a9155a3` — JdbcTemplate 11 method 上层 try-catch translate + 嵌套 try-finally(`lib/spring/jdbc.ss:166-394`)+ phase4_spike 3/3 GREEN
  - Phase 5 `81de9bd` — 7 形态 integration_test.ss(`tests/d139_sql_exception/integration_test.ss:70-251` 7 test)+ absorb 删 Phase 2/4 spike + 主线 close
- **VCM 六验全 PASS**:**§1 工程豁免**(本 Phase `git diff --stat HEAD -- bootstrap/ lib/ tools/` 空输出 — 仅 docs/ + tests/ 改动 — VCM §验 1 豁免条款触发,显式贴 diff stat 空输出证据)+ §2 行为(`grep -cE '^[[:space:]]*test\(' tests/d139_sql_exception/integration_test.ss` 实测 = 7,前 = 1,§核心目标 7 判据 7 个独立 test 兑现)+ §3 反向(撤回 7 case 文件 → §核心目标 Case 1/3/4/5/7 5 项无独立 GREEN test)+ §4 边界(Followup F1-F10 锚明确 + Phase 0-5 commit hash 6 个全核对 + 兑现成果 file:line 锚 7 case 全列)+ §5 路线(D135-D138 + D139 SQL 主线范式延续 — 每 Phase 独立 commit + Status 收关 + commit hash 回填 + next_prompt 自闭环;Phase 5 是 D139 主线 close 标记)+ §6 根因(file:line 锚 = D139 §Phase 收关锚 §Phase 5 mark Done at 本行 + 全 Phase commit hash 6 个总览;PSM 字段 9 全产出标"根",VCM §6 反身验证 — 7 case e2e 测试是 §核心目标 7 判据 root anchor / absorb 决策消除主轴 vs spike 重叠维护负担 / Phase 4 hash 回填消除 §D 文档治理 死指针风险);baseline:tests/d141-d144 inference 5/0/5 + tests/phase5 194/0/4(4 fail pre-existing 与 Phase 1-4 baseline 一致)+ reflection_health GATE PASS no regressions(M2/M3a/M4/M5/M7b/N2/N3 AUTO-DRIFT 1% 软警告不阻;F1 PROGRESS gen_methods 711 / gen_decls 726 — 本 Phase 不动 bootstrap/ 不变)+ d_doc_index GATE OK 12 referenced Ds all live
- D135-D139 SQL 主线范式延续(每 Phase 独立 commit + Status 收关 + commit hash 回填 + next_prompt 自闭环)— **D139 主线 close,后续等用户指定下一 SQL 主线 sub-D**:候选 D138 §Followup F3 cursor / F4 NamedParameterJdbcTemplate / F5 RowMapper<T> memory feedback_d026_d027_phantom_anchor generic 已落 gen_generic_class.ss / F6 HikariCP + SQLRecoverableException / F7 batchUpdate + BatchUpdateException / F1 SESSION_TRACK;D139 §Followup F1 SQLState 完整 ASCII / F2 MySQL errorCode ~150+ / F8 SESSION_TRACK 同位 / F9 DAE.getRootCause / F10 SS closure capture root fix
...
| F5 | RowMapper<T> 泛型 callback + SQLException 整合 | D138 §Followup F5 锚同位 — memory feedback_d026_d027_phantom_anchor — generic 已落 gen_generic_class.ss 14 case GREEN 现可做 — 留独立 sub-D |
...
- 2026-04-29 Phase 5 测试覆盖 7 形态 + 全 Phase 收关 + D139 主线 close(commit `81de9bd`)— 重写 `tests/d139_sql_exception/integration_test.ss` 7 case e2e 全形态:Case 1 wrong-pwd URL → CannotGetJdbcConnectionException(`integration_test.ss:70-87`)+ Case 2 INSERT dup_key → DuplicateKeyException + cause 三字段(`:90-112`,吸收 Phase 2 spike + Phase 4 spike Test 1)+ Case 3 table 不存在 → BadSqlGrammarException + sqlState 42S02 + errorCode 1146(`:114-130`)+ Case 4 NOT NULL violation → DataIntegrityViolationException + sqlState 23000 + errorCode 1048(`:132-150`)+ Case 5 raw java.sql layer `conn.prepareStatement.executeUpdate` catch SQLException 三字段(message + sqlState + errorCode 全可达,`:152-182`,bypass JdbcTemplate translate)+ Case 6 5 层 catch hierarchy DuplicateKey→DataIntegrityViolation→NonTransientDAE→DAE→Error 5 个独立 try-catch 实证(`:184-234`,吸收 Phase 4 spike Test 2 4 层 + 扩 NonTransientDAE 中间 + Error 顶层)+ Case 7 wrong-user URL handshake → CannotGetJdbcConnectionException + sqlState 28000 + errorCode 1045(`:236-251`);**absorb 决策**:删 `tests/d139_sql_exception/phase4_spike_test.ss`(Case 2/6 吸收覆盖 + Test 3 finally-close 已落 `lib/spring/jdbc.ss:166-394` 主轴行为)+ 原 Phase 2 spike `integration_test.ss` Case 2 单 case 已被本 Phase 重写覆盖 — 保留 `translator_spike_test.ss` 12 case 纯本地无 docker(协议层反查表 vs 业务层 catch chain 互补无重叠);**全 Phase 0-5 commit hash 总览**:Phase 0 `68a8184` / Phase 1 `04821ce` / Phase 2 `dd0ef4a` / Phase 3 `be394f5` / Phase 4 `a9155a3` / Phase 5 `81de9bd`;VCM 六验全 PASS — **§1 工程豁免**(`git diff --stat HEAD -- bootstrap/ lib/ tools/` 空输出 — 仅 docs/ + tests/ 改动)+ §2 行为(`grep -cE '^[[:space:]]*test\(' tests/d139_sql_exception/integration_test.ss` = 7 实测,前 = 1)+ §3 反向(撤回 7 case → Case 1/3/4/5/7 5 项无独立 GREEN)+ §4 边界(F1-F10 锚 + Phase 0-5 hash 6 个全核对)+ §5 路线(D135-D139 SQL 主线范式延续 + D139 主线 close)+ §6 根因(全产出标"根",file:line 锚 = D139 §Phase 收关锚 §Phase 5 mark Done + 全 Phase commit hash 6 个总览);baseline:d141-d144 5/0/5 + phase5 194/0/4(4 fail pre-existing)+ reflection_health GATE PASS no regressions(M2/M3a/M4/M5/M7b/N2/N3 AUTO-DRIFT 1% 软警不阻 + F1 PROGRESS gen_methods 711 / gen_decls 726 — 本 Phase 不动 bootstrap/ 不变)+ d_doc_index GATE OK 12 referenced Ds all live;**编译器零改动**(本 Phase 纯 docs/ + tests/);**D139 主线 close,后续等用户分流下一 SQL 主线 sub-D**(D138 §Followup F3 cursor / F4 NamedParameterJdbcTemplate / F5 RowMapper<T> memory feedback_d026_d027_phantom_anchor generic 已落 gen_generic_class.ss / F6 HikariCP + SQLRecoverableException / F7 batchUpdate + BatchUpdateException / F1 SESSION_TRACK;D139 §Followup F1 SQLState 完整 ASCII / F2 MySQL errorCode ~150+ / F8 同位 / F9 DAE.getRootCause / F10 SS closure capture root fix)
- 2026-04-29 Phase 4 JdbcTemplate 11 method 上层 try / catch (e: SQLException) → translator.translate(e) → throw DAE + 嵌套 try-finally 担保 stmt+conn close + queryForList 流式异常 leak-on-failure + spike `jdbc.update INSERT dup_key catch DuplicateKeyException` 3/3 GREEN(commit `a9155a3`)— `lib/spring/jdbc.ss` JdbcTemplate 11 method(execute×2 / update×3 / queryForList×2 / queryForString×2 / queryForInt×2)全 wrap 外层 try-catch translate + 内层嵌套 try-finally close stmt+conn(execute :166-186 / execute(setter) :188-209 / update :210-229 / update(setter) :231-252 / update(setter,kh) :255-289 / queryForList :294-303 / queryForList(setter) :308-318 / queryForString :320-336 / queryForString(setter) :338-354 / queryForInt :356-372 / queryForInt(setter) :374-390);queryForList × 2 为流式异常路径(返回 ResultSet alias socket,caller 持 fd,失败路径 leak — D137 简单 per-call 模型 / HikariCP D125+ pool reclaim 收尾);D137 5 setter method 签名零破 + D138 KeyHolder 第 6 method 契约零破 + ResultSet 透传 caller 契约零破;spike `tests/d139_sql_exception/phase4_spike_test.ss` 3/3 GREEN(docker 在线时实测,离线 fallback skip 沿 d134/d136/d138/Phase 2 模式)— Test 1 业务侧 catch DuplicateKeyException + ex.cause.sqlState "23000" + ex.cause.errorCode 1062 + ex.message "Duplicate entry";Test 2 父类 catch DataIntegrityViolation / 根类 catch DataAccessException 同实例 4 层 hierarchy 兑现;Test 3 dup_key throw 多次后 jdbc.queryForInt(SELECT id) 仍 GREEN — finally-close 担保 socket 不饥饿;**SS try / catch / finally 嵌套实证**:`tests/phase5/finally_basic.ss` test4 "TFC" 链 + `/tmp/test_d139_phase4_finally_return.ss` "TF1F2" 嵌套 finally + `/tmp/test_d139_phase4_throw_path.ss` "TFC:boom" throw 链 + `/tmp/test_d139_phase4_returnpath.ss` D007 return-path 接受 try-return + catch-throw 模式;VCM 六验全 PASS — bootstrap 三阶段固定点(本 Phase lib/ + tests/ 改触 build cache,stage2 == stage3)+ d134_mysql 5/0/5 + d135 1/0/1 + d136 1/0/1 + d138 1/0/1 + d139 3/0/3(Phase 2+3+4 spike 各 1)+ d141-d144 inference 25/0/25(d141 5+d142 6+d143 6+d144 8)+ phase5 194/0/4(4 fail pre-existing 与 Phase 1/2/3 baseline 一致)+ reflection_health GATE PASS no regressions(M2/M3a/M4/M5/M7b/N2/N3 AUTO-DRIFT 1% 软警 + F1 PROGRESS bootstrap/gen/methods/gen_methods.ss 730→728→711)+ d_doc_index GATE OK 12 referenced Ds all live;**本 Phase 仅 wrap 不重写**(JdbcTemplate 11 method 业务逻辑 prepareStatement / setter / executeUpdate / getGeneratedKeys / executeQuery 路径全保留,仅外层加 try-catch + 内层加 try-finally — D137/D138 契约 + ResultSet 透传契约全零破)— Phase 5 7 形态完整测试覆盖留续作主线 close
