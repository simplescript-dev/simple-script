...
4. [x] Done at bootstrap/eval/member_access.ss + bootstrap/eval/interp_obj.ss + bootstrap/gen/stmts/stmts_loop_forin.ss(2026-04-21)— **Execute 3**:member_access.ss 删 `cls.fields` / `cls.annotations` 两条 hardcoded 特例分支,改走 ClassMeta object `.name/.fields/.methods/.annotations` 经 interpGetField 统一读(§决策 4)。interpBuildTypeInfo 补 `.annotations` 填充(AnnotationMeta object + InternPool `ANN|CLS|<cls>.<ann>` dedup)。stmts_loop_forin.ss cls.fields loop 边界补偿:FieldMeta object unwrap 回 string tv 绑 ctVars,让 foldComptimeIdentsInTree 对 `get_${f.name}` 模板的 scalar fold 路径不变。commit `bda5813`。
5. [x] Done at bootstrap/gen/stmts/stmts_loop_forin.ss + bootstrap/eval/interp_obj.ss + bootstrap/gen/class/class_comptime.ss + bootstrap/gen/exprs/exprs.ss + bootstrap/gen/gen_types.ss + bootstrap/parse/prelude.ss(2026-04-21)— **Execute 4**:genForIn 删 4 条反射 sidecar 分支(`.methods` / `.annotations` / `.args` × 2),Meta 数组统一走 ct-probe + evalMemberAccess interpGetField 路径。补 3 处短板:(a) `interp_obj.ss` interpBuildTypeInfo / interpCtFieldsArray 填充 MethodMeta + FieldMeta.annotations(含 AnnotationMeta.args),删 4 条外层 `has` guard(getString 返 "" + 内层 continue 天然过滤);(b) `class_comptime.ss` foldComptimeIdentsInTree 加 MEMBER_ACCESS 分支:IDENT 绑定 Meta object 时经 interpGetField 拿 scalar field 直接 rewrite 为 STRING_LIT(解决 tvKind=object 无法 fold 的断点);(c) `exprs.ss` isCtStringIdx/resolveCtString + `gen_types.ss` inferType 加 ctVars Meta object 分支(method-gen 期 f.name 求值,与 handler 期 fold 互补)。**实测 delta** vs baseline:M2 +378/tol 380 DRIFT(仅剩 2 预算)/ N2 +1890/tol 1903 DRIFT(仅剩 13 预算)/ 结构组 11 项全 OK/PROGRESS / F1 全 OK(gen_decls.ss 690 PROGRESS)**GATE PASS no regressions**。**§新张力 1 "三加必配三删"抵消证据兑现**:Execute 1 prelude.ss 3 类加 (+16 M2) + Execute 2 interpBuildTypeInfo 实现 (+357 M2 增量) 合计 +373 M2,由本轮删 4 条 sidecar 分支 + 4 条 has guard (~-100 M2) 部分抵消。bootstrap 固定点通过,d095/d096 全系列 + l2 覆盖测试全 PASS,4 条 pre-existing 失败无关。
6. [x] Done at bootstrap/eval/interp_obj.ss + bootstrap/gen/exprs/exprs_ct_obj.ss + bootstrap/eval/interp_core.ss + bootstrap/gen/exprs/exprs.ss(2026-04-21)— **Execute 5**:迁 `interpCtFieldsArray` / `interpCollectFields` 调用点改走 evalExpr + 删原函数。**3 处代码迁移**:(a) `interp_obj.ss` interpBuildTypeInfo 吸收 interpCtFieldsArray body + interpCollectFields 父类链 walk,FieldMeta/AnnotationMeta inline 构造 + InternPool `FLD|<cls>.<fld>` / `ANN|FLD|<cls>.<fld>.<ann>` dedup;(b) `exprs_ct_obj.ss:11` 实例构造 **非反射路径** 解耦至 nGetList 父类链直枚举(prepend parent fields first,与 interpCollectFields 语义一致),消除 ctNewExprDispatch 对 eval-layer helper 的调用;(c) `exprs_ct_obj.ss:54/64` TypeValue/object `.fields()` 方法改走 `interpGetField(interpBuildTypeInfo(name), "fields")` 统一经 ClassMeta dedup。**删原函数 2 个**:`interpCollectFields` (13 lines) + `interpCtFieldsArray` (33 lines)。**import 清理**:`interp_core.ss:15` + `exprs.ss:6` 删除两函数 import。**D097 §后续工作 4 "L2ζ-L2κ Map migrate" 落地** + D098 §决策 2 §Phase B "Meta 对象 InternPool 承载" §下一步 [x] Done 回写。
