...
- **[ ] Planned** Phase 1.5d 剩余 prereq:evalExpr 入口双轨彻底消除(ct/runtime 分支合一,真单 dispatch:eager genVal + mv 协议 + 桥消除)+ genBinary/genStringConcat/genValStringCompare 接口扩(take pre-eval reg)+ 回 Phase 1 原目标(do-while / while / for 类 A 消除)+ D093 §0.3 验收 5 项全达成。**接口返回类型保持 int**(D098 §决策 1 §Phase A→B 接口稳定性),不引 class
