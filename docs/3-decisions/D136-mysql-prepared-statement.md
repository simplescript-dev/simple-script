...
- **F1 IR 类型声明顺序**:`bootstrap/gen/codegen.ss` 当前先 emit 全部 lib `.ss` IR 函数体 → 再 emit `%ClassName = type {...}` 声明 → llc 对 lib 间 GEP `%CrossModuleClass` 报 `base element must be sized` forward-ref。Phase 2 用 `query.ss columnDefColType` / `columnDefName` 函数包装 GEP 绕道(参 `lib/com/mysql/query.ss:141-157`)。**根因方案** = 改 codegen IR emit 顺序(类型声明先于函数体)— 影响面:整个 codegen pass 顺序,大改;**跟进开 sub-D `D138 cross-module struct GEP` 评估**;Phase 2 局部绕道是临时方案,不是永久 API
  > **⚠ D138 编号冲突注释**(D137 §Phase 4 落锚 + §F4 follow-up 锚 2026-04-27):本 §F1 引用 "sub-D `D138 cross-module struct GEP`",但 D136 §R4(line 362)同时引用 "留 D138 sub-follow-up handle" 指 prepared statement cache miss handle — **D138 编号双指**(语义截然不同:§R4 = cache miss handle / §F1 = cross-module struct GEP)。D 治理后续轮处理(候选编号方案:D138 = cache miss handle 取 §R4 先到优先 / cross-module GEP 另起新编号如 D141+;或 D138 = cross-module GEP / cache miss 另起);**本轮 D137 Phase 4 仅在此加注释指出冲突,不动编号**(承 D137 §F4 锚:`docs/3-decisions/D137-jdbctemplate-prepared-retcon.md` Followup 表 F4 行)。后续 D 治理轮:(a) 选定真 D138 归属(R4 cache miss vs F1 cross-module GEP),(b) 另一引用 retcon 为新编号,(c) 跑 `bin/ss run tools/d_doc_index_linter.ss` 验证 F1 死指针 = 0 持守 + 引用一致性
...
### Phase 2: COM_STMT_EXECUTE + binary result set + COM_STMT_CLOSE 实施 [✓] Done at commit `1a70844` (2026-04-27)
...
- [✓] **JdbcTemplate retcon scope 评估**:**defer D137 sub-follow-up**(根因校准非降级,承 §核心原则 §Root Cause)
  ...
  - 取舍:R5 风险锚命中"signature 破裂 + scope 爆"双触发,defer D137 是根因决策 — D136 Phase 3 仅落 e2e 验证 binary protocol 全链通,JdbcTemplate API 演化让 D137 单独评估(候选 ① 引入 `Array<Param>` Param 类承载 type code + value;候选 ② 加 method overload 编译器支持;候选 ③ 走 `executeQuery(sql).bindInt(1, x).bindString(2, y).run()` builder pattern — 三候选权衡留 D137 PSM)
- [✓] D134 §Status retcon **不需要**(无 JdbcTemplate prepared 接管 → 无"text protocol superseded"锚要落)
- [✓] GREEN: `bin/ss test tests/d136_prepared_statement/` 1 文件 probe-skip pass(docker unreachable)+ `bin/ss test tests/d134_mysql/` 5 文件全绿 + `bin/ss test tests/d135_caching_sha2/` 1 文件全绿 + `bin/ss test tests/` baseline 不降 + `./build.sh bootstrap` 三阶段固定点 stage2==stage3 byte-identical(纯 tests + docs 改不动 bootstrap)+ axiom 红线 grep / nm = 0 永久 + d_doc_index_linter F1 = 0(D134 §Status 未 retcon → 无 forward ref 风险)
