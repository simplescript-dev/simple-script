...
**Status:** Phase 2 (候选评估 + 决策行不锁定 留用户对话授权) — D149 Phase 0 落档 commit `0852ce9` 后 Phase 1 commit `d1c35e0` RED 复现 + 信息源探查 + scope 实测 + Phase 0 hash 回填 4 处真锚 + 决策行不锁定留 Phase 2 用户对话锁定;D148 bidirectional type checking 主线 close at commit `729b6f1` 后 D149 启动 commit `91f8b80` §F1 Tuple unnecessary GREEN + D149 接续 commit `acdf554` §F2 NEW_EXPR ctor unnecessary GREEN + D149 接续接续 commit `0852ce9` §F3 spike form 1 RED + Phase 0 落档 + D149 Phase 1 commit `d1c35e0` Phase 0 hash 回填 + Phase 1 RED 复现 + 信息源探查 + scope 实测 + D149 Phase 2 commit `<placeholder>` Phase 1 hash 回填 4 处真锚 + 候选评估 C2 接口层 trap vs C3 架构层 refactor + §字段 10 (e) 自决策 + 长久 / 演化维度(底层依赖链 / 业界对标 / N 年返工度)+ Phase 1 新发现 (i) 全 default 空 ctor silent bug 是 Phase 2 决策必须覆盖范畴(单 candidate 必须双路径覆盖 partial + 全 default,否则不合格)— 决策行不锁定留用户对话授权门槛触发(类比 D148 Phase 2 H15 授权范式)— F3 不同 F1/F2 模式(F1/F2 是 bidirectional 自然解度 spike GREEN unnecessary 入 D148 §A.3 废案),F3 是「ctor default value 配套机制独立扩展」spike RED 启动条件触发起 D149 sub-D 主体(D148 §Followup F3 描述精确启动条件)。Phase 1 兑现 a-h 八项 fact 全 GREEN + Phase 2 兑现 a-? 总览 fact 全 GREEN + D135-D148 范式延续。
...
### 决策行(Phase 2 候选评估 fact 入档 + 决策行不锁定 留用户对话授权)

C1 全废(已废入 §A.3)。

**Phase 2 候选评估 fact**(commit `<placeholder>` Phase 2 docs only — 不动 bootstrap/lib/tools):
...
| H12 | Phase 2 决策行不锁定 留用户对话授权门槛(类比 D148 Phase 2 H15 授权范式)| ✓ Phase 2 实证兑现 — 架构层 refactor C3 引入 helper `checkNamedArgs(callId, calleeKind)` 抽象,长久演化影响范围扩到 call / method call / NEW_EXPR ctor 三场景,需用户对话锁定;Phase 2 启动轮不自决策,等用户对话明确锁定后才入 Phase 3 | 用户对话授权未触发 → Phase 2 阻塞,Phase 3 不启动(等用户对话明确锁定 C2 vs C3 才入 Phase 3 改 bootstrap)|
...
### Phase 2: 候选评估 fact 入档 + 决策行不锁定 留用户对话授权 [✓] Done at commit `<placeholder>` (2026-05-03)
...
- **§3 Orchestration 表 Phase 2 行 [✓] Done at commit `<placeholder>` 升级**(候选 1 数据层 patch 全废 + C2 / C3 双候选评估 + 双路径覆盖标记 + §字段 10 (e) 自决策评估)
...
- bootstrap/gen/class/class.ss ctor IR 用 default value 填充未列 field(partial 路径 + 全 default 路径双修 — Phase 1 新发现 (i) silent bug 配套修)
