...
| **Phase 1** [✓ Done at commit `<placeholder>`(留 D151 Phase 2 启动轮回填 — D135-D150 范式 单 commit 不能引用自己 hash 下下轮回填)] | Phase 0 hash `2e94b54` 回填 + 用户对话锁 **C-B-3 底层先做** + 候选范畴细化(D152 底层 type class sub-D 先做 + D151 setter ≥18 method 后做接力) | Phase 0 hash 回填 D151.md 4 行 6 字串 + D147.md 1 行 1 字串 = 5 行 7 字串非均匀分布 line 268 含 3 字串 + 用户对话锁 C-B-3(2026-05-04 Phase 1)+ Phase 2+ 路径明确 = 起 D152 底层 type class sub-D 先做 | 用户对话授权门槛 PASS + Phase 0 hash 回填 GREEN + 候选范畴细化 + 路径分支 C-B-3 |
| **Phase 2+** Planned | C-B-3 锁定后路径明确 — 新建 D152 底层 type class sub-D 先做 + D151 setter ≥18 method 后做接力 | **D151 Phase 2 = 起 D152 底层 type class sub-D**(`lib/java/math.ss` BigDecimal + `lib/java/sql.ss` 加 Timestamp/Date/Time/Blob/Clob/NClob/RowId/SQLXML/Array/Ref + `lib/java/io.ss` 加 InputStream/Reader,~300-1000 LOC);D152 close 后 D151 Phase 3+ 起首 setter ≥18 method(~70-150 LOC + 3 implementor + integration_test) | D152 sub-D Phase 0-N+ close + D151 setter Phase 3-N+ close |
...
### Phase 1: Phase 0 hash 回填 + 用户对话锁 C-B-3 底层先做 + 候选范畴细化 + 路径分支 [✓] Done at commit `<placeholder>`(留 D151 Phase 2 启动轮回填 — D135-D150 范式 单 commit 不能引用自己 hash 下下轮回填)(2026-05-04)
...
- **§3 Orchestration 表 Phase 1 行 [✓ Done at commit `<placeholder>`(留 D151 Phase 2 启动轮回填)]** + **Phase 2+ 行 C-B-3 路径明确化**(D152 底层 sub-D + D151 setter 接力)
...
### Phase 2+ Planned (C-B-3 路径锁定后)

- **D151 Phase 2 = 起 D152 底层 type class sub-D**(新建 D152-jdbc-type-class-foundation.md 或类似命名;`lib/java/math.ss` BigDecimal + `lib/java/sql.ss` 加 Timestamp/Date/Time/Blob/Clob/NClob/RowId/SQLXML/Array/Ref + `lib/java/io.ss` 加 InputStream/Reader)~300-1000 LOC
- **D152 close 后 D151 Phase 3+ 起首 setter ≥18 method**(~70-150 LOC + 3 implementor 同步 ~70 LOC + integration_test 扩 cases ~200 LOC)
