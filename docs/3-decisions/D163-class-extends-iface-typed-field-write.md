...
**Status:** [x] Phase 0 D 文档落档 at commit `eb7bd05` + [x] Phase 1 minimal repro 隔离 spike + 3 case RED-as-spike + LLVM IR 真根因 = MEMBER_ACCESS 漏 ifaceMethodsCG.has(推翻 §A.2 H1 GEP offset 错位假设)at commit `8933f0a` + [x] Phase 2 bootstrap/gen codegen 修法 + 三阶段固定点 stage2==stage3 PASS + Phase 1 spike 3 case 全 GREEN at commit `ae16073` + [/] Phase 3 hold D160 §Phase 4 wire 真值反射重启实测发现独立 PIR liveness bug(helper fn 内 while 循环对 class instance method call 触发每次循环 ss_drop → 第 2 次循环 use-after-free segfault — D163 §A.2 H5 假设"修编译器后 D160 wire drain 代码不动即 GREEN" 破裂,真根因 #2 在 PIR liveness 而非 codegen 类型解析)→ 起首 **D164 sub-D** 修 PIR liveness root cause at commit `<phase3-hold-hash>` + [ ] Phase 3 重启 D164 close 后回 D163 §Phase 3 wire 真值反射重启 + 在线 docker 8 case 全 GREEN at commit `<phase3-restart-hash>` + [ ] Phase 4 D163 主线 close + D160 §F9 重启锚回填 at commit `<phase4-hash>` — D160 §F9 起首脱胎,D162 §F7 vtable + non-0-arg overload child class bug 修法系列同源 corner case。修 SS 编译器 `class Child extends Parent : Iface { box: Box }` 形态下子类 own method 内 `this.box = newBox` 对 interface-typed own field write 写不进 own field 的 codegen bug — 不接受次优 / workaround / 节省路径(用户对话锁)。
...
| 3 hold | D160 §Phase 4 wire 真值反射重启实测发现独立 PIR liveness bug(helper fn + while loop + JdbcTemplate.execute = `ss_drop_JdbcTemplate(tmpl)` 在 while body 内每次循环 drop → 第 2 次循环 use-after-free segfault)→ D163 §A.2 H5 假设破裂 → 起首 D164 sub-D 修 PIR liveness root cause | `<phase3-hold-hash>` |
...
### Phase 3 hold: D160 §Phase 4 wire 真值反射重启发现独立 PIR liveness bug + 起首 D164 [/] Hold at commit `<phase3-hold-hash>` 等 D164 close 后 D163 §Phase 3 重启
