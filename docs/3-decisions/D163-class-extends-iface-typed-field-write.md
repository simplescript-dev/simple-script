...
| Phase | 内容 | Commit |
|-------|------|--------|
| 0 | D 文档落档(§核心目标 + §核心原则 + §A.1-A.3 + §Phase 收关锚 Phase 0-4 + §Followup F1-F? + §Status 时间线)+ D160 §Phase 3 hash `647f5cc` 跨 D 起首回填 | `<phase0-hash>` |
| 1 | minimum repro `tests/d163_class_extends_iface_typed_field_write/phase1_repro_spike_test.ss` ≥3 case + bootstrap LLVM IR inspect 确认 GEP offset 错位 + 对比 D162 §F7 修法系列已修 vtable indirect dispatch 路径 vs 本 D field write 路径 codegen 差异 | `<phase1-hash>` |
| 2 | bootstrap/gen codegen 修法(GEP offset / RC retain 路径补)+ ./build.sh bootstrap 三阶段固定点 stage2==stage3 bit-identical PASS + reflection_health_linter no regression + Phase 1 spike 全 GREEN | `<phase2-hash>` |
...
### Phase 0: D 文档落档 [ ] Pending at commit `<phase0-hash>`
...
### Phase 1: minimum repro 隔离 spike + ≥3 case GREEN + LLVM IR inspect [ ] Pending at commit `<phase1-hash>`

- 移植 `/tmp/d160_min_repro.ss` 至 `tests/d163_class_extends_iface_typed_field_write/phase1_repro_spike_test.ss` ≥3 case(Case 1 single iface field write / Case 2 多 iface field write / Case 3 nested class extends + iface impl)
- LLVM IR inspect:`bin/ss build phase1_repro_spike_test.ss --emit-ir` + grep `this.box = ` 行的 GEP 指令 + 对比预期 offset(parent fields count + own field idx)
- 对比 D162 §F7 修法系列已修 vtable indirect dispatch raw ptr GEP 路径 vs 本 D field write 路径 codegen 差异
- 输出:Phase 2 修法 target file:line list(`bootstrap/gen/...` 哪些函数 / 哪几行需修)
- bootstrap/ 不动(本 Phase 仅 tests/ + 诊断)

### Phase 2: bootstrap/gen codegen 修法 + 三阶段固定点 PASS [ ] Pending at commit `<phase2-hash>`

- 修 `bootstrap/gen/...`(具体 file:line 在 Phase 1 LLVM IR inspect 后定)子类 own method 内 interface-typed own field write GEP offset / RC retain 路径
- `./build.sh bootstrap` 三阶段固定点 stage2==stage3 bit-identical PASS
- reflection_health_linter no regression GATE PASS(Phase 2 必走 — 本 Phase 改 bootstrap/gen)
- Phase 1 spike 重跑 ≥3 case 全 GREEN(field write 后 read 拿新值)
