...
**Status:** Phase 2 — codegen 阶段反推实施 + G1 D141/D142 同模式复刻 [✓] Done at commit `<TBD>` (2026-04-27)— checker check_exprs.ss:285 OBJ_LITERAL untyped 放行(D141 ARROW_FUNC + D142 ARRAY_LIT 同模式不硬错)+ check_types.ss OBJ_LITERAL inferType case 加 fallback "auto" + gen_types.ss 加 helper 三函数(`isClassType` 13 行 / `extractClassName` 11 行 / `inferObjLiteralFromType` 13 行 / `inferObjLiteralFields` 10 行)+ inferType OBJ_LITERAL case 加优先读 nGetS2 fallback "ptr"(8 行)+ gen_calls.ss + gen/methods/gen_methods.ss args 循环加 inferObjLiteralFields(D141/D142 G1 4 落点同模式)+ **§A.2 H10 OOD 实证错估修正**(Phase 1 仅 grep `evalObjLiteral` 字面 handler 0 命中漏判,实际 `eval/call.ss:86 + eval/method_call.ss:89 + eval/new_expr.ss:38` 通用 pre-eval `genVal(callArgId)` 路过 OBJ_LITERAL 走 `[genVal] unknown kind` silent miscompile;Phase 2 修正:三处 pre-eval 之前同模式前移 inferObjLiteralFields — 与 D141 H10 + D142 H10 完全同形)+ **H3 嵌套反推链路**(`gen/class/class_method.ss:genNamedConstructorArgs` 用 classFieldTypes 反推 inner OBJ_LITERAL + `eval/new_expr.ss` pre-eval 前移对偶)+ spike `/tmp/spike_d143_phase2.ss` GREEN(`takesUser({ name: "Y", age: 18 })` 输出 `Y:18` + `takesProfile({ user: { name: "Z", age: 20 }, addr: "Earth" })` 输出 `Z@Earth`,对照 RED `[genVal] unknown kind: OBJ_LITERAL` + `call ptr @takesUserSingle(ptr 0)` IR 错例)+ VCM 六验全 PASS(bootstrap 三阶段固定点 + tests/ 270/4/274 不降 + reflection_health GATE PASS 扩容申报 D143#扩容申报-Phase2 6 metric M1=5450→5550 / M2=80000→81600 / M4=3109→3250 / N2=400000→408000 / N3=550000→562000 / F1:gen_types.ss=915→970 + d_doc_index_linter F1=0 + next_prompt_ultrathink_linter)— D135/D136/D137/D140/D141/D142 范式延续(D142 Phase 2 commit `0339171` 同模式)。Phase 1 完结 commit `f089738` RED 复现 + §A.2 H1 funcParamTypes 时序 PASS + OBJ_LITERAL slot 探查 PASS + Phase 2 入口锁就绪。Phase 0 完结 commit `50912b4` D 文档落档。**Phase 2 完结**(下一步 Phase 3 测试覆盖 + §A.2 H1-H8 全 PASS 实证 + H4 部分字段决策 + H5b interface upcast OOD scope 标 + H9/H11/H12/H13 OOD/弱化标 + Followup F4 NEW_EXPR ctor 实参 OBJ_LITERAL 反推考量)。
**Depends on:**
...
### Phase 2: codegen 阶段反推实施 + G1 路径(D141/D142 G1 同模式复刻)[✓] Done at commit `<TBD>` (2026-04-27)
