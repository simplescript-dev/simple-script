...
### Phase 4: workaround cleanup [✓] Done at commit `<TBD>` (2026-04-27)
...
- D135/D136/D137/D140/D141/D142 范式延续(每 Phase 独立 commit 大改档,Phase 4 cleanup 实施单元 — D141 Phase 4 commit `70f9457` 11 处 + D142 Phase 4 commit `f757aca` 4 处 + D143 Phase 4 commit `<TBD>` 0 处 docs-only 降级,实测发现反推机制覆盖范围下 lib/+tests/ 全 0 cleanup 候选 — 反映 D141/D142 Phase 4 已清理 lambda 类型注解 + ctor (NEW_EXPR) 路径 array literal 临时变量绑定 后,fn/method 实参 OBJ_LITERAL 路径 stdlib + tests/ 既无既有 workaround 候选)
...
- 2026-04-27 Phase 4 workaround cleanup(commit `<TBD>`)— **grep 实测 0 命中**(`grep -rn 'let \w\+: \w\+ = {' lib/ tests/ --include='*.ss' | grep -v 'explicit_override\|object_literal.ss'` = 0,排除 D143 Phase 3 自测 explicit_override.ss + D084 既有 baseline tests/phase5/object_literal.ss);**Phase 4 docs-only 降级**(无 cleanup 实施 — 与 D141 Phase 4 commit `70f9457` 11 处 + D142 Phase 4 commit `f757aca` 4 处不同形;D143 Phase 4 全 0 处 cleanup 候选 → LOC delta = 0,仅 D143 文档改动);**关键发现 D143 反推机制 fn 实参 OBJ_LITERAL 路径 stdlib + tests/ 已无 workaround 候选**(`lib/spring/data.ss` D141 Phase 4 已清理 lambda 类型注解 5 处 + 既无 OBJ_LITERAL fn 实参绑定形态;其他 lib/(json / url / argparse / path / string_utils / 其他)+ tests/(d134_mysql / phase5/ / 其他)同 0 命中 — `let u: User = {...}; takesFn(u)` 形态在反推机制覆盖范围下既无既有候选,亦无后续业务路径残留);范围排除项 — (a) **NEW_EXPR ctor 实参 OBJ_LITERAL** 留 §Followup F4 sub-D(D142 §F7 + D143 §F4 同根因合并锚);(b) **var decl 显式注解 D084 rewrite** 是 D084 既有路径非 workaround;(c) **partial fields / 默认值** 留 §Followup F6 sub-D(Phase 3 H4 LLC error 严格模式决策行落档);(d) **倒置类型 / 无 callee context 粒度过松** 留 §Followup F8 sub-D(Phase 3 H13 实测 segfault + unknown kind LLC error);VCM 六验全 PASS(核心代码路径 diff=0 → VCM §1 豁免锚成立 + tests/d143 6/0/6 + tests/d142 6/0/6 + tests/d141 5/0/5 + reflection_health GATE PASS no regressions 不动 bootstrap 不增量 + d_doc_index PASS 10 referenced Ds all live + ultrathink_linter);LOC delta 0(0 文件改 cleanup,仅 D143 文档落档)— D135/D136/D137/D140/D141/D142 范式延续(D141 Phase 4 commit `70f9457` 11 处 + D142 Phase 4 commit `f757aca` 4 处 + D143 Phase 4 commit `<TBD>` 0 处 docs-only 降级 同模式 lib/+tests/ 全 0 cleanup 候选 — 反映 D141/D142 Phase 4 已清理后 fn/method 实参 OBJ_LITERAL 路径既无既有 workaround 候选)
