...
- 写 `/tmp/spike_array_lit_untyped.ss` 形态:`takesArr([1, 2, 3])` 直接传 fn 实参 + 多形态(单元素 / 多元素同质 / 空数组 / 嵌套 / interface upcast)— 实测 RED 路径
- 跑 `bin/ss build /tmp/spike_array_lit_untyped.ss --emit-ir` 验证 IR 层是否 RED(若已 GREEN → 改 spike 触发反推需求)
- 探查 funcParamTypes Array<T> 字符串注册时机(grep `bootstrap/gen/codegen.ss:110-112` + `gen_registry.ss:56-57`)— **大概率 D141 H1 实证已成立**,Phase 1 走个过场实证记录
- 探查 ARRAY_LIT 节点 slot 当前占用(`bootstrap/parse/parse_exprs.ss:496` newNode + nSetList 后,s1/s2/s3 是否空)— Phase 2 选 slot
- 探查 evalArrayLit pre-eval 时序(对照 D141 H10 同形 — `genVal(elemId)` 在 genArrayLit 之前)— 反推前移落点确认
- §A.2 H1/H10 实证记录 + 新假设(若有)入档
...
- 6+ 测试用例落锚 `tests/d142_array_literal_inference/`:
  - `untyped_single.ss` — H1 单元素反推
  - `untyped_multi.ss` — H2 多元素同质反推
  - `nested_array.ss` — H3 嵌套 array 反推 capture 链
  - `empty_array.ss` — H4 空数组反推
  - `interface_upcast.ss` — H5 interface upcast vtable indirect dispatch
  - `explicit_override.ss` — H6 显式注解 > 推断
- VCM 六验全 PASS
- Phase 2 兑现漏点根因修(若有 — 测试 end-to-end 暴露)合并入 Phase 3 commit(`feedback_root_cause_no_cost.md` 第一法则,不延期 / 不分独立 commit)
