...
**Status:** Plan + Execute 全 Done(Execute 1 at `gen/rt/gen_rt_cache.ss` + Execute 2 at `gen/gen_var_alias.ss` + Execute 3 at `gen/gen_emit.ss` + Execute 4 at `eval/ct_driver.ss:155` + Execute 5 at `gen/class/class_annotation.ss:159`)
...
| **Execute 5** `[x] Done at gen/class/class_annotation.ss:159` | 迁 annotation stub 到 `bootstrap/gen/class/class_annotation.ss` 156 → **169**:annClassNodeIds + annClassAnnNames + registerAnnotation + collectClassAnnotations + emitAnnotationInits(2 array + 3 空函数 + 1 注释)。codegen.ss 删 L59-69 共 11 行,**实测 343 → 332**(R1 PROGRESS -11)。**同步手动清 `tools/linter_baseline.txt:16` `F1:bootstrap/gen/codegen.ss=1166` 漂移条目**(D113 Execute 4 遗留 A 类漂移 -823)。P10.1 top-level **29 条**(22 let + 7 函数)归类 **2 类**(驱动 7 函数 + state registry 22 let)= §决策 2 ≤ 2 类**达标**。4 gate 全绿:bootstrap 固定点通过 / tests 215 passed 4 failed(全 pre-existing:spring_web_params / d096_p4_l2_reactive / harness_bug / harness_task) / `reflection_health_linter` GATE PASS no regressions / P10.1 结构 gate 达标。| ~11 行迁移 | 零 | 低(空 stub,SS 全局 scope 自动接续 resetCodegen L297-298 赋值)|
