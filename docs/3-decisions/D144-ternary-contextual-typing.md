...
**Status:** Phase 0 — D 文档落档 [ ] Planned — D141 §Followup F3 / D142 §Followup F2 / D143 §Followup F1 同模式合并锚扩到 TERNARY 节点(`cond ? a : b` 在 fn 实参 `int?` / `User?` / `class<T>` 等可空 / 结构化 callee 形参时反推两分支统一类型);D141 反推机制 + D142 elemType 反推机制 + D143 className 反推机制同模式扩到 TERNARY — callee PARAM 已结构化 SSoT 复用 + codegen 阶段反推 + eval pre-eval 时序前移 + funcParamTypes 信息源单点;比 D143 略轻 — TERNARY 反推目标不是字段 list 而是两分支(then/else)各自统一类型(信息流向是 callee.expectedType → 两分支 inferType,与 D141/D142/D143 callee.expectedType → 节点 slot 同模式)
**Depends on:**
...
- `check_exprs.ss:TERNARY case` 改返 `branchType` 结构化(与 ARROW_FUNC + ARRAY_LIT + OBJ_LITERAL 同形 + 优先读节点 nGetS?)
- `gen_types.ss` helper 落锚:`isNullableType` + `extractInnerType` + `unifyBranchTypes`(对偶 D141 isFnType + D142 isArrayType + D143 isClassType)
- `gen_calls.ss:resolveCallArgs` + `gen/methods/gen_methods.ss:emitClassMethodCall` 内 args 循环识别 TERNARY + 调 `inferTernaryBranchType(argId, typeCallee, argIdx)` 反推回填(D141 + D142 + D143 G1 4 落点同模式复刻)
- `eval/method_call.ss + eval/call.ss + eval/new_expr.ss` pre-eval 之前反推前移(D141/D142/D143 H10 同模式 — H10 cross-D 反思继承)
- `genTernary` 改:开头读 `nGetS?(id)` 拿反推 branchType + 反推得 branchType 后两分支按 branchType widen / null literal 类型化走 D067 既有机制
- D067 nullable 分配路径复用(反推得 callee `T?` 后 null literal 类型化路径已就绪,不重写)
- 反推 IR 因果实证 spike 形态 — 反推前 IR 报错或 silent miscompile / 反推后 IR phi node 统一 + null literal 走 nullable 路径 GREEN(对照 stash + ./build.sh bootstrap 重 build 双向实测)
- VCM 验证:bootstrap 三阶段固定点 PASS + tests baseline 不降(270/4/274)+ tests/d067 + tests/d141 + tests/d142 + tests/d143 不破 + reflection_health GATE PASS
...
- grep + 删现存 fn 实参 ternary 临时变量绑定 workaround(若有 `let n: int? = (x > 0) ? null : 5; takesFn(n)` 形态)
