...
- **[~] In Progress** Phase 1.5b Execute: eval_expr.ss UNARY/BINARY/TERNARY/NULL_COALESCE 5 处类 B 入口消除(callsite 走 `mvKnownOf`/`mvValOf`/`reg`)
  ...
  - **[ ] Planned** 1.5b 后续子轮 — TERNARY(bootstrap/eval/ternary.ss:10 入口)留 1.5c 同批
- **[ ] Planned** Phase 1.5c Execute: bootstrap/eval/ 其他 ~10 处类 B 入口消除(ternary/short_circuit/index_access/template_lit/array_lit/ident/member_access/postfix_inc/method_call/call/new_expr 各自子模块)+ genBinary/genStringConcat/genValStringCompare 改 take pre-eval reg(消除 1.5b BINARY delegate 路径的 double-eval 桥)
