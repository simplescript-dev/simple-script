...
- **[x] Done at `<本 commit>`(sibling 50 RED 实测重构 probe `/tmp/maybeval_overload.ss` 物理验证 SS 函数重载 int vs ptr by-type dispatch 已物理修复 retro-active 跨时点状态同步收口 — paramSig (`gen_types.ss:835`) + paramSigChecker (`check_types.ss:349`) 两层 mangling 物理生效, IR 证据 `@f_i` + `@f_MaybeVal` callsite 正确路由 dispatch 第七次假设破裂形态首例 — 跨时点 D 文档段 mangling 失效定位状态前进度误判语义层;Phase A 错名访问器 `mvKnownOf/mvValOf` 历史规避方案物理动机消除,sibling 51+ 可考虑统一回 `mvKnown/mvVal` 简化命名空间)** SS 函数重载语义澄清(int vs ptr 同名 dispatch 缺失,见 §新张力 6),不作 Phase A 阻塞项,Phase A 先用 `mvKnownOf/mvValOf` 错名访问器。本 anchor 由 retro-active 跨时点状态同步收口,sibling 第五十例应用(D170 §决策 C exit 动作 §2 D 文档治理 anchor 收口形态续手第七次 — 跨时点 D 文档段 mangling 失效定位状态语义层首手:D098 §新张力 6 文字 2026-04-18 时点 "mangling 失效层 parser/checker/codegen TBD" vs 2026-05-27 物理 paramSig + paramSigChecker mangling 已生效 1+ 月时点漂移收口,RED 实测主动改正第七次形态发现)
