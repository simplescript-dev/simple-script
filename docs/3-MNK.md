...
> 本文档是 SimpleScript 项目八股流程层闸门的**单一事实源**。PSM 十问、VCM 五验、改动分层、收尾 gate、八股自检、rule 问答收敛循环,以及所有前后文流程反射规则,全部收敛在这里。
>
...
4. **表面解决必须记账** —— 机械 linter 只校验格式不判真假时,PSM 字段 10 标"下轮升根路径",不藏表面为根
...
| **大改** | LOC delta > 100 **或** > 5 文件 **或** 新增 D 文档 | 十问全 | 五验全 | 必做 | 全四步 + **独立 commit 禁打包** |
...
**强制**:接到任意任务(含用户明确指定的),**第一次工具调用之前**必须用纯文字把十问 PSM 表格写在回复正文里。
...
| 4 | 规则 | 本任务硬约束(引用 CLAUDE.md / 本文档 / D088 等具体段落) | 另见 **rule 问答收敛循环 M=2 子问卷**(见 §K + §附录 A)凡 commit 标准改 / 大改任务必答,答案贴 VCM §5 (d) 作答 |
| 5 | 界定 | 做什么 + **不做什么**,两清单都填 | 详见下方 **§字段 5 两项扩展义务** |
| 6 | 步骤 | 可执行序列,每步 < 30 min | 超过则递归开新十问 |
| 7 | 验证 | 五验 VCM 的具体填充(见 §N) | |
| 8 | 对照实验 | **不做这件事,§第一性需求 会被卡吗?** | 必答 yes/no + 一句证据;no → 本轮任务**降级为 backlog 不做**(除非用户明说偏离授权) |
| 9 | Plan vs Execute | 本轮产出形态:**Execute**(代码改动)/ **Plan**(方向、清单、调研、巡检) | Plan 型 VCM ④ 边界 替换为「替代方案对比 + 隐藏假设挑战」 |
| 10 | 表面 vs 根 | 本任务**和每个产出项**:根解决 / 表面解决? | **根** → 一句话给出消除的双轨制 / 架构根因;**表面** → 必须同时写「本轮接受表面的成本理由」+「下一轮如何升级到根」;多产出项任务**对每一项**单独标记 |

### 字段 5 两项扩展义务

**(a) 命名前缀族归位扫描**(涉及"创建 `bootstrap/X/<prefix>_Y.ss`"类任务):
...
| 2 | 行为 | 最小新能力代码,**前 ≠ 后**;前段必须复现失败 / 不存在的能力,后段 demo 新能力 | **不许引用现有测试** |
| 3 | 反向 | 删除新实现 → 必须看到失败 | 强制证明因果 |
| 4 | 边界 | 极限 / 异常 / 空 / 类型边界输入(Plan 型替换为「替代方案对比 + 隐藏假设挑战」) | 防 happy path bias |
| 5 | 路线 | (a) 对照 D088 §正模式 / §反模式 / §验证标准 §核心验证 逐条打勾; (b) **第一性需求距离**:本轮产出和 D088 §第一性需求 实现路径间距(用 phase 衡量),≥ 2 phase → 警告"可能绕道",必须论证"为何先做 phase X"; (c) **表面 vs 根复检**:对照 PSM 字段 10 标记,确认没把表面伪装成根; (d) **八股沉积 M-linter-K-M 问答收敛循环**(见 §K) | 防离开主线 + 防八股跨轮沉积 |
