...
- bootstrap/interp.ss (interpreter core + emit/registerFunction/getAnnotatedClasses/getTypeInfo/addStringConst builtins)
- bootstrap/gen_annotations.ss (annotation dispatch — factory + wrapper gen + handler calls)
- lib/spring/boot.ss (comptime factory + wrapper generation block)

## Last Round (max 3 sentences)
Completed D087 Phase 4b: method wrappers now generated by comptime block in lib/spring/boot.ss instead of hardcoded compiler logic. Added addStringConst builtin, interpBuildAnnotationArray helper, and method/class annotation support in @typeInfo. gen_annotations.ss now uses deterministic wrapper names (__ann_wrapper_ClassName_methodName) and skips wrapper generation when comptime already registered it. 184 tests passing, bootstrap fixed-point verified.

## Task
**D087 Phase 4c: Clean up gen_annotations.ss — remove dead factory/wrapper code**

Now that comptime generates both factories AND method wrappers, the hardcoded generation functions in gen_annotations.ss are dead code (only used as fallback when comptime hasn't run). Clean up:

1. **Verify**: Confirm that when Spring Boot's comptime block runs, ALL factory and wrapper functions are already registered in funcRetTypes before emitAnnotationInits() executes
2. **Delete emitFactory()**: The entire function (lines 109-152) — comptime generates these now
3. **Delete emitMethodWrapper()**: The entire function (lines 158-196) — comptime generates these now
4. **Simplify emitAnnotationInits()**: Remove the factory generation loops (lines 206-229) and the wrapper generation conditional (the `if (funcRetTypes.has(wrapperName) == 0)` block). Keep handler dispatch calls (calling springAnnotationHandler with fn pointers) — those are still needed
5. **Remove annClassSet**: Only used for factory dependency detection in emitFactory — no longer needed
6. **Verify tests + bootstrap**: 184 tests passing, fixed-point verified

Design consideration:
- Handler dispatch (the ptrtoint + call handler pattern) MUST stay in gen_annotations.ss — it's the compiler's responsibility to call registered annotation handlers with fn pointers at init time
- The `collectClassAnnotations` and `collectMethodAnnotations` functions MUST stay — they populate the data that handler dispatch uses
- Only remove the IR generation functions (emitFactory, emitMethodWrapper) and their supporting code
...
- **D087 next**: Phase 4c (clean up gen_annotations.ss) → 4d (verify full comptime pipeline)
...
- **Phase 4b wrappers**: same comptime block generates @__ann_wrapper_ClassName_methodName with parameter bridging (HttpServletRequest/Response/PathVariable). addStringConst builtin for IR string refs. gen_annotations.ss skips when funcRetTypes already has the wrapper name
