...
## Role
Senior technical architect. Project: SimpleScript (self-bootstrapping compiled language, ~14223 LOC).
...
6. **Null Safety Phase 3 (D067)** — 操作符类型传播：`?.` 返回 `T?`，`??` 返回 `T`。checker 层面的类型传播。
...
### Project Status
- **D067 Phase 1+2 complete**: T? type syntax, checker null awareness, smart narrowing (5 patterns).
- **File sizes**: checker.ss ~990 (largest), check_stmts.ss ~758, gen_class.ss ~613, parser.ss ~608, gen_decls.ss ~594, parse_exprs.ss ~531.
- **Stdlib**: 21 modules, ~5217 LOC in lib/.
- **Bootstrap**: 35 files, ~14223 LOC, 86 phase5 tests (all passing).
...
- **Priority**: 语言核心改进优先于 stdlib 模块扩展。先完善 null safety Phase 3、access modifiers 等语言特性。
...
- **Null safety Phase 1 (D067)**: `T?` parsed by `maybeNullable()` in parser.ss. Checker: `isNullableType(t)`, `stripNullable(t)`, `isPrimitiveNullable(t)`. `checkerInferType(NULL_LIT)` → `"null"`. `isTypeCompatible` enforces: null→T ERROR, null→T? OK, T?→T ERROR, T→T? OK. Codegen: `stripNullableCG(t)` strips `?` from all type registries (varTypes, funcRetTypes, classFieldTypes). `ssTypeToLLVM("T?")` → `"ptr"`. Ptr comparison for class instances Eq/Ne with null. `genNullCoalesce` uses `icmp eq ptr, null` for non-string types. Primitive nullable (`int?`/`double?`/`bool?`) rejected by checker.
- **Null safety Phase 2 (D067)**: Smart narrowing via `narrowedTypes` Map in checker. `extractNullCheckVar(condId)` detects BINARY Eq/Ne with IDENT + NULL_LIT (either side). `restoreNarrowing(var, old)` manages save/restore. 5 narrowing patterns: (1) `if (x != null)` narrows in then-branch, (2) `if (x == null) {...} else {...}` narrows in else-branch, (3) `if (x == null) { return/throw }` narrows after if (early exit), (4) `if (x != null) {...} else { return/throw }` narrows after if, (5) reversed operand `null != x`. Reassignment resets narrowing (`narrowedTypes.delete` in ASSIGN handler). Per-function fresh Map in FUNC_DECL handler. `checkerInferType(IDENT)` checks `getNarrowedType(name)` before returning declared type.
...
- **phase5 tests**: 86 tests (all passing).
- **35 bootstrap files**, ~14223 LOC.
...
- D067 Phase 1+2 complete: null safety at checker level with smart narrowing. Phase 3 (operator type propagation) is next.
- All remaining open issues are BLOCKED (I001/I002/I005 need struct/enum) or LOW (I006/I008).
- **Primary work stream is now null safety Phase 3, then access modifiers.**
- File sizes: checker.ss ~990 (largest), check_stmts.ss ~758, gen_class.ss ~613, parser.ss ~608, gen_decls.ss ~594.
...
- 35 bootstrap files total, ~14223 LOC, 86 phase5 tests (all passing).
