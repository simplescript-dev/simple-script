...
## Role
Senior technical architect. Project: SimpleScript (self-bootstrapping compiled language, ~15321 LOC).
...
### Project Status
- **D078 complete**: Static fields. `[private|protected] static [const] fieldName: Type [= value]`. Access via `ClassName.field`. Static fields are global variables. Literal init (int/double/bool/string/negated) emitted directly; non-literal queued for `emitStaticFieldInits()` at main() start. Checker: `staticFields` Map, excluded from constructor params, access-modifier enforcement, const enforcement, type checking. `instance.staticField` rejected. PARAM.I4=1 for static flag.
- **D077 complete**: Enum iteration methods. `Color.values()` returns array of variant values (int or string). `Color.names()` returns array of variant name strings. `enumDeclNodes` Map stores enum AST node ID; `genEnumValues`/`genEnumNames` walk AST directly.
- **D076 complete**: Enum string values. `enum Direction { Up = "up" }`. Parser accepts STRING token, codegen returns `addStringConst(value)`, `inferType` returns "string". Mixed int+string is compile error. `enumTypes` Map (Set semantics, only string enums stored, `has()` = string enum).
- **D075 complete**: `as` type casting. `expr as ClassName` at comparison precedence. Runtime-checked via `ss_isinstance`, throws on failure.
- **D074 complete**: `instanceof` operator. `obj instanceof ClassName` at comparison precedence.
- **D073 complete**: Java-style error handling. `finally` block. Built-in `Error` class. `throw(new Error("msg"))`. Typed catch `catch (e: IOError)`.
...
- **File sizes**: checker.ss ~1221 (largest), check_stmts.ss ~937, gen_class.ss ~714, parser.ss ~688.
- **Stdlib**: 21 modules, ~5217 LOC in lib/.
- **Bootstrap**: 35 files, ~15321 LOC, 106 phase5 tests (all passing).
...
- **Class body fields (D061 complete)**: All code uses `class Foo { fields }` syntax. Old `class Foo(fields)` syntax removed from parser. Detection: `const` → field; `private`/`protected` → field or method; `static` → field or method; `IDENT` + `:` or `?` → field; `function`/`@` → method. Body fields create PARAM nodes in CLASS_DECL.List.
...
- **phase5 tests**: 106 tests (all passing).
- **35 bootstrap files**, ~15321 LOC.
...
- **D074 instanceof operator complete**: `obj instanceof ClassName`. Comparison precedence. Uses existing `ss_isinstance` runtime.
- **D075 as type casting complete**: `expr as ClassName`. Comparison precedence. Runtime-checked via `ss_isinstance`, throws on failure. Enables downcasting.
- **D076 enum string values complete**: `enum Direction { Up = "up" }`. String values in enums. Mixed int+string compile error. `enumTypes` Map for type tracking.
- **D077 enum iteration methods complete**: `Color.values()` and `Color.names()`. Compile-time array generation via `genEnumArray()`. `enumVariantNames` Map for variant name registry.
- **D078 static fields complete**: `[private|protected] static [const] fieldName: Type [= value]`. Global variables with literal/deferred init. Full checker validation. Completes class-level state support.
...
- **Enum fully featured**: int values, string values, values()/names() iteration (D076+D077).
...
- 35 bootstrap files total, ~15321 LOC, 106 phase5 tests (all passing).
