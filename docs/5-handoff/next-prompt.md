...
## Role
Senior technical architect. Project: SimpleScript (self-bootstrapping compiled language, ~13865 LOC).
...
### 项目状态
- **文件大小**: checker.ss ~852, check_stmts.ss ~616, gen_class.ss ~613, gen_decls.ss ~593, parser.ss ~559, parse_exprs.ss ~531, parse_stmts.ss ~519, gen_runtime.ss ~507, gen_calls.ss ~507.
- **Stdlib**: 20 modules, ~4862 LOC in lib/.
- **Bootstrap**: 35 files, ~13865 LOC, 79 phase5 tests.
...
- **phase5 tests**: 79 tests.
- **35 bootstrap files**, ~13865 LOC.
...
- I003 Phase 3 complete + Phase 4 partial: type inference + 5 type check sites + builtin method types (D059). Phase 4 remainder: full inferType migration to checker.
- File sizes: checker.ss ~852 (largest), check_stmts.ss ~616, gen_class.ss ~613, gen_decls.ss ~593.
...
- **Builtin method types now in checker (D059)**: string/Array/Map/Set/Math methods have return types. String methods also have param types. `resolveCheckerClass` handles generic→base normalization. `inferCheckerClass` covers 8 node kinds including literals.
...
- 35 bootstrap files total, ~13865 LOC, 79 phase5 tests.
