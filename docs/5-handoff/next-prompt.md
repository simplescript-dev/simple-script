...
## Role
Senior technical architect. Project: SimpleScript (self-bootstrapping compiled language, ~14144 LOC).
...
6. **Null Safety Phase 2 (D067)** — 智能收窄：`if (x != null)` then-branch 中 x 自动从 T? 收窄为 T。`if (x == null) { return }` 后续代码中 x 也收窄。scope-based narrowing，重新赋值重置收窄。
7. **Null Safety Phase 3 (D067)** — 操作符类型传播：`?.` 返回 `T?`，`??` 返回 `T`。checker 层面的类型传播。
8. **Access Modifiers** — TS 有 `export/private/protected`，模块可见性控制
9. **Standard library expansion**: 延后，语言核心完善后再做
10. **Phase 4 remaining**: Tagged templates (low priority).

### Project Status
- **D067 Phase 1 complete**: T? type syntax, checker null awareness, codegen nullable handling, primitive nullable rejection.
- **File sizes**: checker.ss ~976 (largest), check_stmts.ss ~693, gen_class.ss ~613, parser.ss ~608, gen_decls.ss ~594, parse_exprs.ss ~531, gen_types.ss ~462.
- **Stdlib**: 21 modules, ~5217 LOC in lib/.
- **Bootstrap**: 35 files, ~14144 LOC, 85 phase5 tests (all passing).
...
- **Priority**: 语言核心改进优先于 stdlib 模块扩展。先完善 null safety Phase 2/3、access modifiers 等语言特性。
...
- **phase5 tests**: 85 tests (all passing).
- **35 bootstrap files**, ~14144 LOC.
...
- D067 Phase 1 complete: null safety at checker level. Phase 2 (smart narrowing) and Phase 3 (operator type propagation) are next.
- All remaining open issues are BLOCKED (I001/I002/I005 need struct/enum) or LOW (I006/I008).
- **Primary work stream is now null safety Phase 2, then Phase 3, then access modifiers.**
- File sizes: checker.ss ~976 (largest), check_stmts.ss ~693, gen_class.ss ~613, parser.ss ~608, gen_decls.ss ~594.
...
- 35 bootstrap files total, ~14144 LOC, 85 phase5 tests (all passing).
