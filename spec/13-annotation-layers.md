...
```
ss/web          → @RestController, @GetMapping, @PostMapping, ...
ss/di           → @Service, @Component, @Configuration
dev/db             → @Table, @Id, @Column, @ForeignKey, ...
encoding/json      → @JsonProperty, @JsonIgnore, @JsonFormat
ss/validation   → @Valid, @NotBlank, @Email, @Min, @Max
ss/cache        → @Cacheable, @CacheEvict
dev/db/tx          → @Transactional
dev/log            → @Slf4j
ss/auth         → @RequiresRole, @RequiresPermission
...
import { RestController, GetMapping, PostMapping } from "ss/web"
